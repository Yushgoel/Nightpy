...
#ifdef f2cFortran
#define CFC_(UN,LN)            _(LN,__)     /* Lowercase FORTRAN symbols.     */
#else
...
#define    STRVOID_cfN(T,A)   STRING_cfN(T,A) /* CRAY insists on arg.'s here. */
...
#define    STRVOID_cfSEP(T,B) INT_cfSEP(T,B)
...
#define        STRVOID_cfINT(N,A,B,X,Y,Z)         PVOID_cfINT(N,A,B,X,Y,Z)
...
#define  STRVOID_cfH(S,U,B) STRING_cfH(S,U,B)
...
#define        STRVOID_cfSTR(N,T,A,B,C,D,E) _(CFARGS,N)(T,STRVOID,A,B,C,D,E)
...
#define        STRVOID_cfT(M,I,A,B,D)           A->dsc$a_pointer,A->dsc$w_length
#else
#ifdef CRAYFortran
...
#define        STRVOID_cfT(M,I,A,B,D)           _fcdtocp(A),_fcdlen(A)
#else
...
#define        STRVOID_cfT(M,I,A,B,D)           A,D
#endif
#endif
